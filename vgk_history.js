...
// Changes all relevant buttons and pages to specified year
const yearChange = (e, currElement) => {
  ...
  changeColor(buttonList, "var(--gold-color)", "var(--gray-color)", "active");

  changeColor(lineList, "var(--gold-color)", "var(--gray-color)", "active");

  let prevIndex = parseInt(
    Array.prototype.indexOf.call(buttonList, prevElement)
  );
  let currIndex = parseInt(
    Array.prototype.indexOf.call(buttonList, currElement)
  );
...
const animateLine = (prevElement, currElement, lineList, buttonList) => {
  ...
  if (prevIndex < currIndex) {
    for (let i = prevIndex; i < currIndex + 1; i++) {
      ...
      lineList[
        2 * i
      ].style.transition = `background-color ${duration}s ease ${delay}s`;
      duration += 0.02;
      delay += 0.02;
      buttonList[
        i
    ...
    buttonList[
      currIndex
    ].style.transition = `background-color ${duration}s ease ${delay}s, scale ${duration}s linear ${delay}s`;
